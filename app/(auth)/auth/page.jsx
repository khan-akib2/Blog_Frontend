...
const inputCls = "w-full border-b border-gray-700 bg-transparent py-3 pr-8 text-sm text-white placeholder-gray-600 focus:border-blue-500 focus:outline-none transition-colors";
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" className="inline-block mr-1.5 flex-shrink-0">
...
function AuthContent() {
  ...
  const handleGoogle = useCallback(async (cr) => {
    setGLoading(true);
    try {
      const user = await googleLogin(cr.credential);
      if (user.role === 'admin') { router.push('/admin-login'); return; }
  ...
  // ── Google link (subtle, below divider) ───────────────────────────────────
  const GoogleLink = (
    <button type="button" onClick={triggerGoogle} disabled={gLoading}
      className="flex items-center justify-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors disabled:opacity-50 mx-auto mt-1">
      {gLoading ? <Loader2 className="h-3 w-3 animate-spin"/> : <GoogleIcon/>}
  ...
  const LoginForm = (
    <>
      <h1 className="mb-1 text-5xl font-bold text-white font-serif">Login</h1>
      ...
      <form onSubmit={handleLogin} className="space-y-8">
        ...
        <div className="relative">
          ...
          <button type="button" onClick={()=>setShowPw(!showPw)} className="absolute right-0 top-3">
            {showPw ? <EyeOff className="h-4 w-4 text-gray-600"/> : <Lock className="h-4 w-4 text-gray-600"/>}
          </button>
        </div>
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3"><div className="flex-1 h-px bg-gray-800"/><span className="text-xs text-gray-500">or</span><div className="flex-1 h-px bg-gray-800"/></div>
          {GoogleLink}
        </div>
        <p className="text-center text-sm text-gray-500">
          Don&apos;t have an account?{' '}
          <button type="button" onClick={()=>switchMode('register')} className="font-semibold text-blue-400 hover:text-blue-300 transition-colors">Sign Up</button>
        </p>
      </form>
    </>
  );
  ...
  const RegisterForm = (
    <>
      <h1 className="mb-1 text-5xl font-bold text-white font-serif">Sign Up</h1>
      ...
      <form onSubmit={handleRegister} className="space-y-7">
        ...
        <p className="text-center text-sm text-gray-500">
          ...
          <button type="button" onClick={()=>switchMode('login')} className="font-semibold text-blue-400 hover:text-blue-300 transition-colors">Login</button>
        </p>
      </form>
    </>
  );
  ...
  const OtpForm = (
    <>
      <button onClick={()=>setMode('register')} className="mb-8 inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-white transition-colors">
        <ArrowLeft className="h-4 w-4"/> Back
      </button>
      <h1 className="mb-1 text-5xl font-bold text-white font-serif">Verify email</h1>
      <p className="mb-2 text-sm text-gray-500">We sent a 6-digit code to</p>
      <p className="mb-10 text-sm font-semibold text-white">{regForm.email}</p>
      <div className="flex justify-center gap-3 mb-8" onPaste={handleOtpPaste}>
        {otp.map((digit,i) => (
          <input key={i} ref={el=>inputRefs.current[i]=el} type="text" inputMode="numeric" maxLength={1} value={digit}
            ...
            className={`w-10 h-12 text-center text-xl font-bold bg-transparent border-b-2 text-white focus:outline-none transition-colors ${digit?'border-blue-500':'border-gray-700 focus:border-blue-500'}`}/>
        ))}
      </div>
      ...
      <p className="text-center text-sm text-gray-500">
        ...
        {countdown>0
          ...
          : <button onClick={handleResend} disabled={resending} className="font-semibold text-blue-400 hover:text-blue-300 disabled:opacity-50">{resending?'Sending…':'Resend code'}</button>
        }
      </p>
      <p className="text-center text-xs text-gray-600 mt-2">Code expires in 10 minutes</p>
    </>
  );
    ...
    <div className="relative flex min-h-screen w-full overflow-hidden bg-[#0a0a0f]">
      ...
      <div className="hidden md:block w-full min-h-screen">
        ...
        <div className="absolute top-0 bottom-0 z-20"
          ...
          {/* Diagonal mask — extends 2px beyond edge to kill the artifact line */}
          <div className="absolute z-10 bg-[#0a0a0f]"
            style={{
              top: 0, bottom: 0,
              left: isLogin ? 0 : 'auto',
      ...
      {/* ── MOBILE ── */}
      <div className="md:hidden flex flex-col items-center justify-center min-h-screen w-full px-8 py-16">
